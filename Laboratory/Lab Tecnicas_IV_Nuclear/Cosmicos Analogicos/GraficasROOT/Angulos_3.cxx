...
void Angulos_hacer(const char* filename, const char* pdf) {
    ...
    TF1* fexp2 = new TF1("fexp2", "[0] + [1]*pow(cos(x*TMath::Pi()/180), 2)+[2]pow(cos(x*TMath::Pi()/180), [3]))", xmin, xmax);
    ...
    double d2 = fexp2->GetParameter(2);
    double ed2 = fexp2->GetParError(2);
    double c2 = fexp2->GetParameter(3);
    double ec2 = fexp2->GetParError(3);
    ...
    fitbox2->AddText(Form("c = %.3g #pm %.2g", d2, ed2));
    ...
    legend->AddEntry(fexp2, "Ajuste:a + b cos^{2}(#theta)+ c cos^{n}(#theta))", "l");
    legend->Draw();

    gr->GetXaxis()->SetLimits(-2, 92);
...
int Angulos_3() {
    ...
    const char* pdf1="../Graficas/Angulos_5.pdf";
    Angulos_hacer( archivo1,pdf1);

    const char* archivo2="Angulos_2.csv";
    const char* pdf2="../Graficas/Angulos_6.pdf";
