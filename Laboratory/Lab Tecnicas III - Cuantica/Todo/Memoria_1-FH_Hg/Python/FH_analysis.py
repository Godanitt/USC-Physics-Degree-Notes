...
for df, color, label in zip(dfs, colors, labels):
    f=i/6
    colorog=(0.1+f,0.16+f/3,0.6)
    color=(0.95-f,0.3-f/3,0.4)
    plt.plot(df["U1/V"], df["IA/nA"],"." ,color=color, label=label,markersize="0.5" )
    i+=1
plt.xlabel("$U_1$ (V)")
plt.ylabel("I (nA)")
plt.legend(markerscale=20.0)
plt.savefig("FH_Hg.pdf",bbox_inches="tight")
