...
\begin{equation}
	\Hcal = \Hcal (\rn^N, \pn^N) = E_c (\pn^N) + \varphi(\rn^N) \label{Ec:02-01-02}
\end{equation}
donde $E_c$ es la energía cinética y $\varphi$ la potencial. La densidad de estados $\omega$, que nos da una medida de cuantos estados hay en un intervalo diferencial de energía, viene dada por:
...
\begin{equation}
	L[\Omega] = C \int \D \rn^N \D \pn^N \frac{1}{s} e^{-sE_c}e^{-s\varphi} \label{Ec:02-01-09}
\end{equation}
...
\begin{equation}
	L[\Omega] = C  \parentesis{\frac{2\pi m}{s}}^{f/2} \int \D \rn^N \frac{1}{s} \exp [-s\varphi] \label{Ec:02-01-11}
\end{equation}
...
\begin{equation}
	\Omega = C \frac{1}{\Gamma[f/2+1]} \int \D \rn^N (E-\varphi)^{f/2} \theta [E-\varphi]
\end{equation}
...
\begin{equation}
	\omega = \frac{C}{\Gamma[f/2]} \int \D \rn^N (E-\varphi)^{f/2-1} \theta [E-\varphi]
\end{equation}
Así, para una cantidad $A(\rn^N)$ que solo dependa de las posiciones: 

\begin{equation}
	\langle A \rangle = C \frac{1}{\Gamma[f/2] \omega} \int \D \rn^N (E-\varphi)^{f/2-1} A(\rn^N) \theta [E-\varphi]
\end{equation}
o lo que es lo mismo

\begin{equation}
	\langle A \rangle = \frac{\int \D \rn^N (E-\varphi)^{f/2-1} A(\rn^N) \theta [E-\varphi]}{\int \D \rn^N (E-\varphi)^{f/2-1}\theta [E-\varphi]}
\end{equation}
...
\begin{equation}
	kT = \frac{\Gamma[f/2]}{\Gamma[f/2+1]} \frac{\int \D \rn^N (E-\varphi)^{f/2}\theta [E-\varphi] }{\int \D \rn^N (E-\varphi)^{f/2-1}\theta [E-\varphi]}
\end{equation}
Podemos identificar $E_c = E - \varphi$, tal que $(E-\varphi)^{f/2} = (E-\varphi)^{f/2-1} (E-\varphi)$ de tal modo que la expresión de la derecha no resulta otra cosa que la media de la energía cinética, tal que 
...
La presión  media se calcula a través del teorema de virial y equipartición \cite{ElementosSimluacion} pág. 12. El teorema de equipartición nos dice que en el esamble microcanónico 
\begin{equation}
	p V = N k_B T + \langle \varphi_V \rangle 
\end{equation}
y el teorema del virial nos afirma que 

\begin{equation}
	\langle \varphi_V \rangle = \frac{1}{3} \langle \sum_{i=0}^N \rn_i \cdot \Fn_i \rangle
\end{equation}
donde $\rn_i$ y $\Fn_i$ son la posición de la partícula $i$ y la fuerza sobre la partícula $\Fn_i$ debido a todas las restantes. Lógicamente también se puede obtener a través de las relaciones anteriores y los valores medios sobre el espacio de fases, pero puede llegar a ser bastante complicado. Nosotros hemos deotado
...
En la anterior sección hemos obtenido las relaciones de algunas de las propiedades termodinámicas en el ensamble microcanónico, es decir, a través de la mecánica estadística. Sin embargo, la información generada en la dinámica molecular es la posición y velocidad de cada partícula \cite{ElementosSimluacion}, es decir conocemos la trayectoria de un punto $\Gamma$ del espacio de fases en función del tiempo. Tendremos que asumir la \textbf{hipótesis ergódica} para poder aceptar que el valor medio de nuestro observable $\Acal$ a lo largo de un tiempo suficiente, en nuestro caso al ser una simulación un número de pasos suficientemente grande (asociando a cada paso un tiempo), de tal modo que asumimos
\begin{equation*}
	\Ec^* = \frac{1}{k_{\text{pasos}}} \sum_{k=1}^{k_{\text{pasos}}} E_c^*(t+k\cdot\Delta t)
\end{equation*}
si $k_{\text{pasos}}$ es lo \textit{suficientemente grande}. Una vez asumimos la hipótesis ergódica podemos podemos obtener los valores medios de la ecuación (\ref{Ec:02-01-30}), tras estos 500K pasos podremos obtener un valor de las propiedades de interés (\ref{Ec:02-01-31}), y luego los valores de (\ref{Ec:02-01-32}). Sin embargo lo que nosotros vamos a realizar no son solo 500K pasos, si no que haremos una simulación de 5M (es probable que acabemos haciendo más) de pasos divida en 10 interacciones de 500K pasos, obteniendo entonces 10 valores de la temperatura, 10 valores de la presión... 

Ahora la pregunta que nos surge es: ¿Por qué hacemos esto así? Para obtener un valor medio podríamos coger y hacer los 5 o 10 millones de pasos seguidos, y el resultado (medio) sería exactamente el mismo. Pues bien, esto se hace porque todo experimento debe tener una incertidumbre. Cuando nosotros medimos en el laboratorio la presión somos quienes de darle, con mayor o menor certeza, una incertidumbre a la medida o a los resultados inferidos tras una muestra. Sin embargo nosotros en la simulación no podemos hacer eso, por lo que la única manera de hacerlo es suponer que los valores medios de las propiedades termodinámicas obtenidas con 500K pasos son equivalentes a una medida experimental, de tal manera que nosotros trabajaremos \textit{como si} lo que tenemos fuera una muestra de 10 valores de la variable termodinámica X, de lo que inferir una media $\bar{X}$ y una incertidumbre $s(X)$. Entonces las propiedades termodinámicas derivadas (\ref{Ec:02-01-32}) se calcularán, en realidad, como valores inferidos de la muestra, y por tanto la incertidumbre tendrá que ser calculada con la ecuación de propagación de errores, que abordaremos en el siguiente 
apartado \ref{Subsec:03-01}.

Pero, al igual que en el laboratorio la incertidumbre tiene un origen (estadístico, sistemático...), aquí también debe haber un origen de la incertidumbre. Uno de las más obvias fuentes del error es la hipótesis ergódica. Hemos asumido que en un tiempo suficiente pasamos por todos las posiciones del espacio fásico (y que pasamos el mismo número de veces), pero al limitar el tiempo , esto realmente no ocurre: cada partícula recorre una trayectoria fásica diferente en cada uno de los 500k pasos, lo que hace que haya una diferencia en el valor medio. Otras fuentes de error los errores de Verlet y computacionales, llamados \textit{error de truncación} y \textit{error de redondeo} respectivamente, que ya hemos tratado en \cite{Entrega2} pág. 3-4.  
...
Una vez tenemos todo esto obtener los resultados es cosa de ejecutar los archivos y presentarlos. En las tablas \ref{Tab:01},\ref{Tab:02} y \ref{Tab:03} obtenemos los valores medios y la incertidumbre asociada a los valores medios, mientras que en la tabla \ref{Tab:04} presentamos los valores <<experimentales>> obtenidos.

\begin{table}[h!] \centering
\include{Tabla1.tex}
\caption{Valores medios de las energías y la incertidumbre asociada ($2\cdot \sigma$).}
\label{Tab:01}
\end{table}
\begin{table}[h!] \centering
\include{Tabla2.tex}
\caption{Valores medios de las propiedades (\ref{Ec:02-01-31}) y la incertidumbre asociada ($2\cdot \sigma$).}
\label{Tab:02}
\end{table}
\begin{table}[h!] \centering
\include{Tabla3.tex}
\caption{Valores medios de las propiedades (\ref{Ec:02-01-32}) y y la incertidumbre asociada ($2\cdot \sigma$).}
\label{Tab:03}
\end{table}
...
En el presente documento hemos desarrollado desde la mecánica estadística para el ensamble microcanónico como era la expresión de las propiedades termodinámicas más interesantes en función de valores que son obtenibles a partir de la información que conocemos en nuestra simulación dinámica molecular, y luego a través de la hipótesis ergódica hemos relacionado estos valores medios del ensamble con los valores medios temporales en variables reducidas, que son los que realmente conocemos, junto sus incertidumbres. Así hemos obtenido los valores medios para un gas Lennard-jones genéricos en la colectividad NVE.

Lógicamente ahora tendríamos que ver si los valores medios de los gases coinciden  (por ejemplo mediante la hipótesis nula) con los valores aquí obtenidos para poder ver si ese gas puede ser descrito mediante un potencial Lennard-Jones, que sería el objetivo principal de nuestra simulación.
