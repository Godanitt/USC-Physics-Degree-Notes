...
\begin{itemize}
	\item Hay que hablar de las variables reducidas.
	\begin{equation}
		\frac{kT}{\varepsilon} = T^* = \frac{2\bar{E}^*_{cin}}{\nu} =  \frac{2\bar{E}_{cin}/\varepsilon}{\nu} 
	\end{equation}
	\begin{equation}
		C_V^* = \frac{C_V}{k} = \frac{1}{1+\parentesis{\frac{2}{\nu}-1}\langle E_c \rangle ^* \langle E_c^{-1}\rangle^*}
	\end{equation}
	\begin{equation}
	P^* = \frac{P}{\varepsilon \sigma^3} =  \frac{NT^*}{V^*} - \langle \varphi_V  \rangle^*  = \frac{N^*\frac{kT}{\varepsilon} \varepsilon}{\frac{V}{\sigma ^3} \sigma^3} -  \langle \varphi_V \rangle^*  
	\end{equation}
	\begin{equation}
		\alpha_E^* = \frac{\alpha_E}{k/\varepsilon} = \frac{1}{V^* \ccorchetes{\parentesis{1-\frac{2}{\nu}} \langle E_c  \rangle ^* \langle \varphi E_c^{-1} \rangle^* - \langle \varphi_V \rangle^* }}
	\end{equation}
	La gamma de Brunesen
	\begin{equation}
		\gamma = \frac{NK}{C_V} + V \parentesis{\frac{\nu}{2}-1} \ccorchetes{\langle \varphi_V \rangle \langle E_c^{-1} \rangle  - \langle \varphi_V E_C^{-1} \rangle } \rightarrow
	\end{equation}
	\begin{eqnarray}
		\gamma = \gamma ^* = \frac{N}{C_V/k} + \frac{V}{\sigma ^3} \parentesis{\frac{\nu}{2}-1} \ccorchetes{\langle \varphi_V \rangle \langle E_c^{-1} \rangle  - \langle \varphi_V E_C^{-1} \rangle } \sigma^3 \cdot \frac{1}{\sigma^3} = \\ \frac{N}{C_V ^*} + V^* \parentesis{\frac{\nu}{2}-1} \ccorchetes{\langle \varphi_V  \rangle^* \langle E_c^{-1} \rangle^*  - \langle \varphi_V E_C^{-1} \rangle^* }
	\end{eqnarray}
	El valor de $k_s$ tiene un valor (modulo de ks)
	
	\begin{equation}
		 \begin{split}
		\frac{1}{k_s} = & \frac{NkT}{V} \ccorchetes{1+2\gamma  - \frac{Nk}{C_V}} + V \langle \varphi_{VV} \rangle \\ & - V\parentesis{\frac{\nu}{2} -1 } \ccorchetes{\langle \varphi_V^2 E_c^{-1} -2 \langle \varphi_V \rangle^2 \langle \varphi_V E_c^{-1} \rangle  + \langle \varphi_V \rangle^2 \langle E^{-1} \rangle }
		\end{split}
	\end{equation}
	\item Hay que guardar también la cinética total y la potencial total (media).
	\item La incertidumbre se cacula a partir de los valores medios (10 temperaturas, 10Cvs..) y los hacemos que sean experimentales. Entonces la sigma (debemos aplicar 2 sigma por recomendación, aunque 3 también esta bien, suponiendo que es gaussiano, aunque no lo sea). Mencionar t student, ver estadistica.
\end{itemize}
